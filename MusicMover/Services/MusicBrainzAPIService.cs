...
public class MusicBrainzAPIService
{
    ...
    public async Task<MusicBrainzArtistReleaseModel?> GetReleaseWithAllAsync(string musicBrainzReleaseId)
    {
        using (await _asyncAcoustIdLock.LockAsync())
        {
            ...
            string url =
                $"https://musicbrainz.org/ws/2/release/{musicBrainzReleaseId}?inc=artists+release-groups+url-rels+media+recordings&fmt=json";
    ...
    public async Task<MusicBrainzRecordingQueryModel?> SearchReleaseAsync(string artist, string album, string trackname)
    {
        using (await _asyncAcoustIdLock.LockAsync())
        {
            ...
            string url =
                $"https://musicbrainz.org/ws/2/recording?fmt=json&inc=isrcs+artists+releases+release-groups+url-rels+media+recordings&query=track:\"{trackname}\" AND artist:\"{artist}\"";
            using RestClient client = new RestClient(url);
